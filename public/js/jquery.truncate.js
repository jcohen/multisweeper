 ...
 * 2) Using three optional arguments (deprecated!):
 * width = the desired pixel width, integer
 * tail = text/html to append when truncating
 * treat_as_html = true means treat the text as html text else treat as plain text (preserving markup)
 * width = if not set, will use the width of the container of the text element
 *
...
(function ($) {
	...
	$.fn.textTruncate = function() {
		...
		return this.each(function () {
			 ...
			 **/
			if ( false && options.tail == "..." && func._native ) {
			...
			var pad_left = parseInt( $this.css("padding-left").replace( "px", ''));
			var pad_right =  parseInt( $this.css("padding-right").replace( "px", ''));
			var pad_total = pad_left + pad_right;
			var width = options.width || ( $this.parent().width() - pad_total );
			...
			if ($td.width() > width)
			{
				...
				while ($td.width() >= targetWidth )
				{
					...
					if ( options.treat_as_html )
						...
						$td.text($td.text().substring(0, textlength));
				}
